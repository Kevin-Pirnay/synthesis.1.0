...
export class View_Choose_Root_Use_case
{
    ...
    public handle(request : View_Choose_Root_Request) : View_Choose_Root_Response
    {
        ...
        const choose_root : IChoose_Roots_Root = this.__repository.get_choose_roots_root_injector(indexes);

        choose_root.rotate(request.direction);

        const dtos : IDto[] = choose_root.get_dtos(indexes);
